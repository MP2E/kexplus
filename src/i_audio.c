...
#endif
{
	...
	if (!dstrcmp(driver, "jack") ||
	    ...
	    !dstrcmp(driver, "dart") || !dstrcmp(driver, "file")
	    )
		return;

	CON_Warnf("Invalid driver name\n");
	CON_Warnf
	    ("Valid driver names: jack, alsa, oss, pulseaudio, coreaudio, dsound, portaudio, sndio, sndman, dart, file\n");
	CON_CvarSet(cvar->name, DEFAULT_FLUID_DRIVER);
}
...
static void Seq_SetReverb(doomseq_t * seq,
			  float size, float damp, float width, float level)
{
...
static channel_t *Song_AddTrackToPlaylist(doomseq_t * seq, song_t * song,
					  track_t * track)
{
	int i;
...
static void Event_NoteOn(doomseq_t * seq, channel_t * chan)
{
	...
	fluid_synth_noteon(seq->synth, chan->track->channel, chan->key,
			   chan->velocity);
}
...
static void Event_PitchBend(doomseq_t * seq, channel_t * chan)
{
	...
	fluid_synth_pitch_bend(seq->synth, chan->track->channel,
			       ((b2 << 8) | b1) >> 1);
}
...
static int Signal_Resume(doomseq_t * seq)
{
	...
	SEMAPHORE_LOCK()
		...
		if (c->song && c->paused) {
			c->paused = false;
			fluid_synth_noteon(seq->synth, c->track->channel,
					   c->key, c->velocity);
		}
	}
	SEMAPHORE_UNLOCK()
...
static dboolean Song_RegisterTracks(song_t * song)
{
	...
	song->tracks =
	    (track_t *) Z_Calloc(sizeof(track_t) * song->ntracks, PU_STATIC, 0);
	data = song->data + 0x0e;
...
static dboolean Seq_RegisterSongs(doomseq_t * seq)
{
	...
	seq->songs =
	    (song_t *) Z_Calloc(seq->nsongs * sizeof(song_t), PU_STATIC, 0);
...
void I_InitSequencer(void)
{
	...
	//
	doomseq.driver =
	    new_fluid_audio_driver(doomseq.settings, doomseq.synth);
...
#ifdef _WIN32
	doomseq.sfont_id =
	    fluid_synth_sfload(doomseq.synth, s_soundfont.string, 1);
...
void I_StartMusic(int mus_id)
{
	...
	for (i = 0; i < song->ntracks; i++) {
		chan =
		    Song_AddTrackToPlaylist(&doomseq, song, &song->tracks[i]);
