...
d_inline static dboolean P_CheckThingCollision(mobj_t * thing)
{
	fixed_t blockdist;
...
d_inline
    static void P_BlockMapBox(fixed_t * bbox, fixed_t x, fixed_t y,
			      mobj_t * thing)
{
	...
	bbox[BOXRIGHT] =
	    (tmbbox[BOXRIGHT] - bmaporgx + extent) >> MAPBLOCKSHIFT;
	bbox[BOXBOTTOM] =
	    (tmbbox[BOXBOTTOM] - bmaporgy - extent) >> MAPBLOCKSHIFT;
	...
	if (in->isaline) {
		...
		if (li->flags & ML_TWOSIDED && li->backsector) {
			...
			if ((li->frontsector->floorheight ==
			     ...
			     || (slope =
				 FixedDiv(openbottom - shootz,
					  dist)) <= aimslope)
			    && (li->frontsector->ceilingheight ==
				li->backsector->ceilingheight
				|| (slope =
				    FixedDiv(opentop - shootz,
					     dist)) >= aimslope))
				return true;	// shot continues
		}

		if (!
		    (hitz > sidesector->floorheight
		     && hitz < sidesector->ceilingheight))
			hitplane = true;
		...
		if (hitplane == true) {
			...
			den =
			    FixedDot(plane->a, plane->b, plane->c, shootdirx,
				     shootdiry, shootdirz);

			if (den != 0) {
				fixed_t hitdist;
				fixed_t num = FixedDot(plane->a, plane->b,
						       plane->c, trace.x,
						       trace.y,
						       shootz) + plane->d;
				...
				x = trace.x +
				    FixedMul(FixedMul(aimpitch, trace.dx),
					     frac);
				y = trace.y +
				    FixedMul(FixedMul(aimpitch, trace.dy),
					     frac);
				z = hitz;
			}
		} else {
...
static dboolean P_CheckUseHeight(line_t * line, mobj_t * thing)
{
	...
	if (!(line->flags & ML_SWITCHX02 ||
	      line->flags & ML_SWITCHX04 || line->flags & ML_SWITCHX08))
	...
	if (line->flags & ML_CHECKFLOORHEIGHT) {
		...
		check =
		    (line->backsector->ceilingheight +
		     sides[line->sidenum[0]].rowoffset) + (32 * FRACUNIT);
	else
		return true;
...
dboolean PTR_UseTraverse(intercept_t * in)
{
	...
	if (!(in->d.line->special & MLU_USE)
	    || !P_CheckUseHeight(in->d.line, usething))
		return false;
	...
	if (!usecontext) {
		P_UseSpecialLine(usething, in->d.line,
				 (P_PointOnLineSide
				  (usething->x, usething->y, in->d.line) == 1));
	}
...
static dboolean PTR_ChaseCamTraverse(intercept_t * in)
{
	...
	if (in->isaline) {
		...
		if (side->midtexture == 1 || side->toptexture == 1
...
void P_CheckChaseCamPosition(mobj_t * target, mobj_t * camera, fixed_t x,
			     fixed_t y)
{
	tmcamera = camera;

	if (P_PathTraverse
	    (target->x, target->y, x, y, PT_ADDLINES, PTR_ChaseCamTraverse)) {
		camera->x = x;
		camera->y = y;
	}
}
