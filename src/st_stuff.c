...
void ST_Init(void)
{
	...
	for (i = 0; i < MAXPLAYERS; i++) {
		if (playeringame[i] && net_player_names[i][0])
			snprintf(player_names[i], MAXPLAYERNAME, "%s",
				 net_player_names[i]);
	}
...
static void ST_DrawChatText(void)
{
	...
	for (i = 0; i < MAXCHATNODES; i++) {
		if (stchat[current].msg[0] && stchat[current].tics) {
			...
			Draw_Text(STCHATX, y, stchat[current].color, 0.5f,
				  false, stchat[current].msg);
			y -= 8;
		}
...
static void ST_DisplayName(int playernum)
{
	...
	// get distance
	distance =
	    P_AproxDistance(viewx - player->mo->x, viewy - player->mo->y);
	...
	xpitch =
	    (FixedMul(dsin(viewpitch), yangle) - FixedMul(dcos(viewpitch), z));
	ypitch =
	    (FixedMul(dsin(viewpitch), z) + FixedMul(dcos(viewpitch), yangle));
	...
	screenx =
	    ((FixedDiv(xangle, yangle) * (SCREENWIDTH / 2)) >> FRACBITS) +
	    (SCREENWIDTH / 2);
	screeny =
	    (((FixedDiv(ypitch, xpitch) * -(SCREENWIDTH / 2)) >> FRACBITS) +
	     (SCREENWIDTH / 2)) - 40;
