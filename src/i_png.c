		    ...
		    int *w, int *h, int *offset, int palindex)
{
	...
	if (usingGL && !alpha) {
		...
		if (num_trans)
			...
			I_Error
			    ("I_PNGReadData: RGB8 PNG image (%s) has transparency",
			     lumpinfo[lump].name);
	}
	...
	row_pointers =
	    (byte **) Z_Malloc(sizeof(byte *) * height, PU_STATIC, 0);
...
{
	pngWriteData =
	    (byte *) Z_Realloc(pngWriteData, pngWritePos + length, PU_STATIC,
			       0);
