...
static void saveg_setup_mobjwrite(void)
{
	...
	// allocate ref table
	savegmobj =
	    (savegmobj_t *) Z_Alloca(sizeof(savegmobj_t) * savegmobjnum);
	i = 0;
...
static void saveg_read_laserthinker_t(laserthinker_t * laserthinker)
{
	...
	while (1) {
		if (head) {
			head = false;
			laserthinker->laser =
			    (laser_t *) Z_Calloc(sizeof(laser_t), PU_LEVSPEC,
						 NULL);
...
struct {
...
} saveg_specials[] = {
	{
		T_MoveCeiling,
		    ...
		    saveg_read_ceiling_t, sizeof(ceiling_t)
	}
	, {
		T_VerticalDoor,
		    tc_door,
		    saveg_write_vldoor_t, saveg_read_vldoor_t, sizeof(vldoor_t)
	}
	, {
		T_MoveFloor,
		    tc_floor,
		    saveg_write_floormove_t,
		    saveg_read_floormove_t, sizeof(floormove_t)
	}
	, {
		T_PlatRaise,
		    tc_plat,
		    saveg_write_plat_t, saveg_read_plat_t, sizeof(plat_t)
	}
	, {
		T_LightFlash,
		    tc_flash,
		    saveg_write_lightflash_t,
		    saveg_read_lightflash_t, sizeof(lightflash_t)
	}
	, {
		T_StrobeFlash,
		    tc_strobe,
		    saveg_write_strobe_t, saveg_read_strobe_t, sizeof(strobe_t)
	}
	, {
		T_Glow,
		    tc_glow,
		    saveg_write_glow_t, saveg_read_glow_t, sizeof(glow_t)
	}
	, {
		T_FireFlicker,
		    tc_flicker,
		    saveg_write_fireflicker_t,
		    saveg_read_fireflicker_t, sizeof(fireflicker_t)
	}
	, {
		T_CountdownTimer,
		    tc_delay,
		    saveg_write_delay_t, saveg_read_delay_t, sizeof(delay_t)
	}
	, {
		T_LookAtCamera,
		    tc_aimcam,
		    saveg_write_aimcamera_t,
		    saveg_read_aimcamera_t, sizeof(aimcamera_t)
	}
	, {
		T_MovingCamera,
		    tc_movecam,
		    saveg_write_movecamera_t,
		    saveg_read_movecamera_t, sizeof(movecamera_t)
	}
	, {
		T_MobjFadeThinker,
		    tc_fade,
		    saveg_write_mobjfade_t,
		    saveg_read_mobjfade_t, sizeof(mobjfade_t)
	}
	, {
		T_Sequence,
		    tc_sequence,
		    saveg_write_sequenceGlow_t,
		    saveg_read_sequenceGlow_t, sizeof(sequenceGlow_t)
	}
	, {
		T_Quake,
		    tc_quake,
		    saveg_write_quake_t, saveg_read_quake_t, sizeof(quake_t)
	}
	, {
		T_Combine,
		    tc_combine,
		    saveg_write_combine_t,
		    saveg_read_combine_t, sizeof(combine_t)
	}
	, {
		T_LaserThinker,
		    tc_laser,
		    saveg_write_laserthinker_t,
		    saveg_read_laserthinker_t, sizeof(laserthinker_t)
	}
	, {
		T_MoveSplitPlane,
		    tc_split,
		    saveg_write_splitmove_t,
		    saveg_read_splitmove_t, sizeof(splitmove_t)
	}
	, {
		T_LightMorph,
		    tc_morph,
		    saveg_write_lightmorph_t,
		    saveg_read_lightmorph_t, sizeof(lightmorph_t)
	}
	, {
		T_MobjExplode,
		    tc_exp, saveg_write_mobjexp_t,
		    saveg_read_mobjexp_t, sizeof(mobjexp_t)
	}
	, {
	NULL, tc_endthinkers, NULL, NULL, 0}
};
...
void P_ArchiveSpecials(void)
{
	...
	for (th = thinkercap.next; th != &thinkercap; th = th->next) {
		...
		for (i = 0; saveg_specials[i].type != tc_endthinkers; i++) {
			if (th->function.acp1 ==
			    (actionf_p1) saveg_specials[i].function.acp1) {
	...
	while (1) {
		...
		for (i = 0; saveg_specials[i].type != tc_endthinkers; i++) {
			if (tclass == saveg_specials[i].type) {
				saveg_read_pad();
				thinker =
				    Z_Malloc(saveg_specials[i].structsize,
					     PU_LEVEL, NULL);
				...
				if (((thinker_t *) thinker)->function.acp1 !=
				    NULL) {
					specialthinker = saveg_read32();
					if (specialthinker)
						macrothinker =
