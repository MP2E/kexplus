...
static void M_SetOptionValue(int choice, float min, float max, float inc,
			     cvar_t * cvar);
...
void M_DrawRegion(void)
{
	...
	Draw_BigText(RegionDef.x + 136, RegionDef.y + LINEHEIGHT * region_mode,
		     color, string);
	...
	Draw_BigText(RegionDef.x + 136, RegionDef.y + LINEHEIGHT * region_lang,
		     color, string);
	...
	Draw_BigText(RegionDef.x + 136, RegionDef.y + LINEHEIGHT * region_blood,
		     color, string);
...
#undef DRAWNETWORKITEM
	...
	if (inputEnter) {
		...
		i = ((int)(160.0f / NetworkDef.scale) -
		     Center_Text(inputString)) * 2;
		Draw_BigText(NetworkDef.x + i + 198,
			     (NetworkDef.y + LINEHEIGHT * y), MENUCOLORWHITE,
			     "/r");
	}
	...
	case misc_menufade:
		if (choice) {
			if (m_menufadetime.value < 80.0f)
				M_SetCvar(&m_menufadetime,
					  m_menufadetime.value + 0.8f);
			else
	...
	case misc_cursorsize:
		if (choice) {
			if (m_cursorscale.value < 50)
				M_SetCvar(&m_cursorscale,
					  m_cursorscale.value + 0.5f);
			else
...
void M_DrawMisc(void)
{
	...
	if (currentMenu->menupageoffset <= misc_menufade + 1 &&
	    (misc_menufade + 1) - currentMenu->menupageoffset <
	    currentMenu->numpageitems) {
		y = misc_menufade - currentMenu->menupageoffset;
		M_DrawThermo(MiscDef.x, MiscDef.y + LINEHEIGHT * (y + 1), 80,
			     m_menufadetime.value);
	}

	if (currentMenu->menupageoffset <= misc_cursorsize + 1 &&
	    (misc_cursorsize + 1) - currentMenu->menupageoffset <
	    currentMenu->numpageitems) {
		y = misc_cursorsize - currentMenu->menupageoffset;
		M_DrawThermo(MiscDef.x, MiscDef.y + LINEHEIGHT * (y + 1), 50,
			     m_cursorscale.value);
	}
...
void M_DrawMouse(void)
{
	M_DrawThermo(MouseDef.x, MouseDef.y + LINEHEIGHT * (mouse_sensx + 1),
		     MAXSENSITIVITY, v_msensitivityx.value);
	M_DrawThermo(MouseDef.x, MouseDef.y + LINEHEIGHT * (mouse_sensy + 1),
		     MAXSENSITIVITY, v_msensitivityy.value);

	M_DrawThermo(MouseDef.x, MouseDef.y + LINEHEIGHT * (mouse_accel + 1),
		     20, v_macceleration.value);

	Draw_BigText(MouseDef.x + 144, MouseDef.y + LINEHEIGHT * mouse_look,
		     MENUCOLORRED, msgNames[(int)v_mlook.value]);
	Draw_BigText(MouseDef.x + 144, MouseDef.y + LINEHEIGHT * mouse_invert,
		     MENUCOLORRED, msgNames[(int)v_mlookinvert.value]);
	Draw_BigText(MouseDef.x + 144,
		     MouseDef.y + LINEHEIGHT * mouse_yaxismove, MENUCOLORRED,
	...
	case 0:
		...
		case mouse_sensx:
			if (v_msensitivityx.value > 0.0f)
				M_SetCvar(&v_msensitivityx,
					  v_msensitivityx.value - slope);
			else
				CON_CvarSetValue(v_msensitivityx.name, 0);
			break;
		case mouse_sensy:
			if (v_msensitivityy.value > 0.0f)
				M_SetCvar(&v_msensitivityy,
					  v_msensitivityy.value - slope);
			else
	...
	case 1:
		...
		case mouse_sensx:
			if (v_msensitivityx.value < (float)MAXSENSITIVITY)
				...
				CON_CvarSetValue(v_msensitivityx.name,
		...
		case mouse_sensy:
			if (v_msensitivityy.value < (float)MAXSENSITIVITY)
				...
				CON_CvarSetValue(v_msensitivityy.name,
						 (float)MAXSENSITIVITY);
			break;
		}
		break;
...
void M_ChangeMouseAccel(int choice)
{
	...
	case 0:
		if (v_macceleration.value > 0.0f)
			M_SetCvar(&v_macceleration,
				  v_macceleration.value - slope);
		else
...
void M_DrawDisplay(void)
{
	...
	M_DrawThermo(DisplayDef.x,
		     DisplayDef.y + LINEHEIGHT * (dbrightness + 1),
		     MAXBRIGHTNESS, i_brightness.value);
	Draw_BigText(DisplayDef.x + 140, DisplayDef.y + LINEHEIGHT * messages,
		     MENUCOLORRED, msgNames[(int)m_messages.value]);
	Draw_BigText(DisplayDef.x + 140, DisplayDef.y + LINEHEIGHT * statusbar,
		     MENUCOLORRED, hudtype[(int)st_drawhud.value]);
	Draw_BigText(DisplayDef.x + 140,
		     DisplayDef.y + LINEHEIGHT * display_flash, MENUCOLORRED,
		     flashtype[(int)st_flashoverlay.value]);
	Draw_BigText(DisplayDef.x + 140,
		     DisplayDef.y + LINEHEIGHT * display_damage, MENUCOLORRED,
		     msgNames[(int)p_damageindicator.value]);
	Draw_BigText(DisplayDef.x + 140,
		     DisplayDef.y + LINEHEIGHT * display_weapon, MENUCOLORRED,
		     msgNames[(int)st_showpendingweapon.value]);
	Draw_BigText(DisplayDef.x + 140,
		     DisplayDef.y + LINEHEIGHT * display_stats, MENUCOLORRED,
		     msgNames[(int)st_showstats.value]);

	if (st_crosshair.value <= 0) {
		Draw_BigText(DisplayDef.x + 140,
			     DisplayDef.y + LINEHEIGHT * display_crosshair,
			     MENUCOLORRED, msgNames[0]);
	} else {
		ST_DrawCrosshair(DisplayDef.x + 140,
				 DisplayDef.y + LINEHEIGHT * display_crosshair,
				 (int)st_crosshair.value, 1, MENUCOLORWHITE);
	}

	M_DrawThermo(DisplayDef.x,
		     DisplayDef.y + LINEHEIGHT * (display_opacity + 1), 255,
		     st_crosshairopacity.value);
...
void M_ChangeOpacity(int choice)
{
	...
	if (choice) {
		if (st_crosshairopacity.value < 255.0f)
			M_SetCvar(&st_crosshairopacity,
				  st_crosshairopacity.value + slope);
		else
...
void M_DrawVideo(void)
{
	...
	if (currentMenu->menupageoffset <= video_dgamma + 1 &&
	    (video_dgamma + 1) - currentMenu->menupageoffset <
	    currentMenu->numpageitems) {
		y = video_dgamma - currentMenu->menupageoffset;
		M_DrawThermo(VideoDef.x, VideoDef.y + LINEHEIGHT * (y + 1), 20,
			     i_gamma.value);
	}
...
void M_DrawPassword(void)
{
	...
	if (passInvalid) {
		...
		if (passInvalidTic & 16) {
			Draw_BigText(-1, 240 - 80, MENUCOLORWHITE,
				     "Invalid Password");
			return;
		}
	}
	...
	for (i = 0; i < 19; i++) {
		...
		Draw_BigText((currentMenu->x + (i * 12)) - 48, 240 - 80,
			     MENUCOLORRED, password);
	}
}
...
void M_DrawSound(void)
{
	M_DrawThermo(SoundDef.x, SoundDef.y + LINEHEIGHT * (sfx_vol + 1), 100,
		     s_sfxvol.value);
	M_DrawThermo(SoundDef.x, SoundDef.y + LINEHEIGHT * (music_vol + 1), 100,
		     s_musvol.value);
	M_DrawThermo(SoundDef.x, SoundDef.y + LINEHEIGHT * (gain + 1), 2,
		     s_gain.value);
}
...
void M_Features(int choice)
{
	...
	showfullitemvalue[0] = showfullitemvalue[1] = showfullitemvalue[2] =
	    false;
}
...
void M_DrawFeaturesMenu(void)
{
	...
	/*Lock Monsters Mode */
	M_DrawSmbString(msgNames[(int)sv_lockmonsters.value], &featuresDef,
			features_lockmonsters);

	/*Wireframe Mode */
	M_DrawSmbString(msgNames[wireframeon], &featuresDef,
			features_wireframe);

	/*Invulnerable */
	M_DrawSmbString(msgNames
			[players[consoleplayer].cheats & CF_GODMODE ? 1 : 0],
			&featuresDef, features_invulnerable);

	/*No Clip */
	M_DrawSmbString(msgNames
			[players[consoleplayer].cheats & CF_NOCLIP ? 1 : 0],
			&featuresDef, features_noclip);

	/*Map Everything */
	M_DrawSmbString(msgNames[amCheating == 2 ? 1 : 0], &featuresDef,
			features_mapeverything);

	/*Full Health */
	M_DrawSmbString(showfullitemvalue[0] ? "100%%" : "-", &featuresDef,
			features_healthboost);

	/*Full Weapons */
	M_DrawSmbString(showfullitemvalue[1] ? "100%%" : "-", &featuresDef,
			features_weapons);

	/*Full Keys */
	M_DrawSmbString(showfullitemvalue[2] ? "100%%" : "-", &featuresDef,
			features_securitykeys);

	switch (map->type) {
	case 0:
		sprintf(featuresDef.menuitems[features_levels].name,
			FEATURESWARPLEVEL);
		break;
	case 1:
		sprintf(featuresDef.menuitems[features_levels].name,
...
void M_DoFeature(int choice)
{
	...
	case features_weapons:
		...
		for (i = 0; i < NUMAMMO; i++)
			players[consoleplayer].ammo[i] =
			    players[consoleplayer].maxammo[i];
	...
	case xgp_sensitivity:
		if (choice) {
			if (i_rsticksensitivity.value < 0.0125f)
				M_SetCvar(&i_rsticksensitivity,
					  i_rsticksensitivity.value + slope1);
			else
				CON_CvarSetValue(i_rsticksensitivity.name,
						 0.0125f);
		} else {
	...
	case xgp_threshold:
		if (choice) {
			if (i_rstickthreshold.value < 100)
				M_SetCvar(&i_rstickthreshold,
					  i_rstickthreshold.value + slope2);
			else
...
void M_DrawXGamePad(void)
{
	M_DrawThermo(XGamePadDef.x,
		     XGamePadDef.y + LINEHEIGHT * (xgp_sensitivity + 1), 100,
		     i_rsticksensitivity.value * 8000.0f);

	M_DrawThermo(XGamePadDef.x,
		     XGamePadDef.y + LINEHEIGHT * (xgp_threshold + 1), 50,
		     i_rstickthreshold.value * 0.5f);

	Draw_BigText(XGamePadDef.x + 128, XGamePadDef.y + LINEHEIGHT * xgp_look,
		     MENUCOLORRED, msgNames[(int)v_mlook.value]);

	Draw_BigText(XGamePadDef.x + 128,
		     XGamePadDef.y + LINEHEIGHT * xgp_invert, MENUCOLORRED,
...
void M_BuildControlMenu(void)
{
	...
	for (item = 0; item < actions; item++) {
		...
		if (PlayerActions[item].action) {
			...
			G_GetActionBindings(&menu->menuitems[item].name[16],
					    PlayerActions[item].action);
		} else {
			menu->menuitems[item].status = -1;
...
void M_DrawControlMenu(void)
{
	if (ControlMenuDef.hints[itemOn] != NULL) {
		GL_SetOrthoScale(0.5f);
		Draw_BigText(-1, 410, MENUCOLORWHITE,
			     ControlMenuDef.hints[itemOn]);
...
void M_DrawSave(void)
{
	...
	for (i = 0; i < load_end; i++) {
		...
		Draw_BigText(SaveDef.x, SaveDef.y + LINEHEIGHT * i,
			     MENUCOLORRED, string);
	}

	if (inputEnter) {
		i = ((int)(160.0f / SaveDef.scale) -
		     Center_Text(inputString)) * 2;
		Draw_BigText(SaveDef.x + i,
			     (SaveDef.y + LINEHEIGHT * saveSlot) - 2,
			     MENUCOLORWHITE, "/r");
	}
}
...
static void M_DoDefaults(int choice)
{
	...
	for (i = 0; currentMenu->defaultitems[i].mitem != NULL; i++)
		CON_CvarSetValue(currentMenu->defaultitems[i].mitem->name,
				 currentMenu->defaultitems[i].mdefault);
...
static void M_CheckDragThermoBar(event_t * ev, menu_t * menu)
{
	...
	scaley = ((float)video_height / (float)SCREENHEIGHT) * menu->scale;
...
static void M_DrawThermo(int x, int y, int thermWidth, float thermDot)
{
	...
	Draw_BigText(x +
		     (int)(thermDot * slope) * (symboldata[SM_THERMO].w / 100),
		     y, MENUCOLORWHITE, "/s");
}
...
static dboolean M_SetThumbnail(int which)
{
	...
	//
	if (!P_QuickReadSaveHeader
	    (P_GetSaveGameName(which), thumbnail_date, (int *)data,
	...
	if (thumbnail == 0) {
		...
		dglTexImage2D(GL_TEXTURE_2D,
			      0,
			      GL_RGB8,
			      128, 128, 0, GL_RGB, GL_UNSIGNED_BYTE, data);
	} else {
		dglBindTexture(GL_TEXTURE_2D, thumbnail);

		GL_SetTextureFilter();

		dglTexSubImage2D(GL_TEXTURE_2D,
				 0,
				 0,
				 0, 128, 128, GL_RGB, GL_UNSIGNED_BYTE, data);
	}
...
static void M_DrawSaveGameFrontend(menu_t * def)
{
	...
	if (M_SetThumbnail(itemOn)) {
		...
		Draw_BigText(def->x + 444, def->y + 244, MENUCOLORWHITE,
			     thumbnail_date);

		sprintf(string, "Skill: %s", NewGameMenu[thumbnail_skill].name);
		Draw_BigText(def->x + 444, def->y + 268, MENUCOLORWHITE,
			     string);

		sprintf(string, "Map: %s",
			P_GetMapInfo(thumbnail_map)->mapname);
		Draw_BigText(def->x + 444, def->y + 292, MENUCOLORWHITE,
			     string);
...
void M_DrawXInputButton(int x, int y, int button)
{
	...
	GL_Set2DQuad(vtx,
		     ...
		     xinputbutons[index].h, tx1, tx2, ty1, ty2, color);
...
dboolean M_Responder(event_t * ev)
{
	...
	case KEY_ENTER:
		if (itemOn != itemSelected && ev->type == ev_mousedown)
			return false;
...
static void M_DrawMenuSkull(int x, int y)
{
	...
	GL_Set2DQuad(vtx,
		     ...
		     vy1,
		     symboldata[index].w,
		     symboldata[index].h, tx1, tx2, ty1, ty2, color);
...
void M_Drawer(void)
{
	...
	max =
	    (currentMenu->numpageitems ==
	     -1) ? currentMenu->numitems : currentMenu->numpageitems;
	...
	for (i = start; i < max + start; i++) {
		...
		if (currentMenu->menuitems[i].status != -1) {
			...
			//
			if (currentMenu == &PasswordDef && gametic & 4
			    && i == itemOn)
				continue;
			...
			if (!currentMenu->smallfont) {
				...
				if (itemSelected == i)
					color =
					    D_RGBA(255, 255, 0, menualphacolor);
	...
	if (currentMenu->numpageitems != -1) {
		if (currentMenu->menupageoffset) {
			//up arrow
			Draw_BigText(currentMenu->x, currentMenu->y - 24,
				     MENUCOLORWHITE, "/u More...");
		}

		if (currentMenu->menupageoffset + currentMenu->numpageitems <
		    currentMenu->numitems) {
			//down arrow
			Draw_BigText(currentMenu->x,
				     (currentMenu->y - 2 +
				      (currentMenu->numpageitems -
				       1) * height) + 24, MENUCOLORWHITE,
				     "/d More...");
		}
	}
	//
	...
	if (currentMenu == &PasswordDef) {
		Draw_BigText((currentMenu->x + ((itemOn & 7) * height)) - 4,
			     currentMenu->y + ((int)(itemOn / 8) * height) + 3,
			     MENUCOLORWHITE, "/b");
	} else {
		// DRAW SKULL
		if (!currentMenu->smallfont) {
			...
			if (itemOn) {
				for (i = itemOn; i > 0; i--) {
					if (currentMenu->menuitems[i].status ==
					    -3)
						offset++;
				}
			}
...
void M_Ticker(void)
{
	...
	//
	if (currentMenu == &PauseDef)
		currentMenu->menuitems[pause_features].status =
		    p_features.value ? 1 : -3;

	//
	// hidden hardcore difficulty option
	//
	if (currentMenu == &NewDef)
		currentMenu->menuitems[nightmare].status =
		    p_features.value ? 1 : -3;
	...
	if (currentMenu == &ControlMenuDef)
		currentMenu->menuitems[controls_gamepad].status =
		    xgamepad.connected ? 1 : -3;
#endif
	...
	if (currentMenu->numpageitems != -1) {
		if (itemOn >=
		    (currentMenu->numpageitems + currentMenu->menupageoffset)) {
			currentMenu->menupageoffset =
			    (itemOn + 1) - currentMenu->numpageitems;
	...
	if (thermowait != 0 && currentMenu->menuitems[itemOn].status == 3 &&
		...
		currentMenu->menuitems[itemOn].routine(thermowait ==
						       -1 ? 1 : 0);
	}
}
