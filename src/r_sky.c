...
#define NUM_SKY_DOME_FACES  32
	...
	//
	dglTranslated(-((float)radius / ((float)NUM_SKY_DOME_FACES / 2.0f)),
		      -((float)radius / (M_PI / 2)), -offset);
...
static void R_DrawSimpleSky(int lump, int offset)
{
	...
	GL_SetState(GLSTATE_BLEND, 1);
	GL_SetupAndDraw2DQuad(0, (float)offset - lumpheight, SCREENWIDTH,
			      lumpheight, pos1, width + pos1, 0.006f, row,
			      WHITE, 1);
	GL_SetState(GLSTATE_BLEND, 0);
}
...
static void R_DrawTitleSky(void)
{
	...
	Draw_GfxImage(63, 25, sky->backdrop, D_RGBA(255, 255, 255, logoAlpha),
		      false);
}
...
static void R_SpreadFire(byte * src1, byte * src2, int pixel, int counter,
			 int *rand, int width)
{
	int randIdx = 0;
	byte *tmpSrc;
	...
	if (pixel != 0) {
		...
		*rand = ((*rand + 2) & 0xff);
		...
		tmpSrc =
		    (src1 + (((counter - (randIdx & 3)) + 1) & (width - 1)));
...
static void R_DrawFire(void)
{
	...
	if (!t) {
		...
		dglTexImage2D(GL_TEXTURE_2D,
			      ...
			      FIRESKYSIZE,
			      FIRESKYSIZE,
			      0, GL_RGBA, GL_UNSIGNED_BYTE, firetexture);
	} else {
		//
		// update texture data
		//
		dglTexSubImage2D(GL_TEXTURE_2D,
				 ...
				 FIRESKYSIZE,
				 FIRESKYSIZE,
				 GL_RGBA, GL_UNSIGNED_BYTE, firetexture);
	}
	...
	if (sky->flags & SKF_VOID) {
		...
		if (sky->flags & SKF_CLOUD) {
			...
			if (r_skybox.value <= 0) {
				...
				GL_SetTextureUnit(0, true);
				GL_BindGfxTexture(lumpinfo[skypicnum].name,
						  true);
