...
static void GetModuleName(void)
{
	...
	if (GetModuleFileName
	    (NULL, moduleFileName, charcount(moduleFileName) - 2) <= 0)
...
static void LogPrintf(LPCTSTR fmt, ...)
{
	...
	if (logidx > LOG_BUFFER_SIZE - 1024) {
		WriteFile(logFile, logbuffer, lstrlen(logbuffer), &bytecount,
			  0);
		logidx = 0;
	}
...
static void PrintHeader(void)
{
	...
	if (VirtualQuery
	    ((void *)contextRecord->Eip, &memoryInfo, sizeof(memoryInfo))) {
		if (GetModuleFileName
		    ((HINSTANCE) memoryInfo.AllocationBase, crashModulePath,
		     charcount(crashModulePath) - 2) > 0) {
			crashModuleFn = ExtractFileName(crashModulePath);
		}
	}

	LogPrintf(_T
		  ("%s caused %s Exception (0x%08x)\r\nin module %s at %04x:%08x.\r\n\r\n"),
		  moduleName,
		  PhraseForException(exceptionRecord->ExceptionCode),
		  exceptionRecord->ExceptionCode, crashModuleFn,
...
static void PrintOSInfo(void)
{
	...
	if (GetVersionEx(&osinfo)) {
		...
		wsprintf(mmb, _T("%u.%u.%u"), majorVersion, minorVersion,
			 buildNumber);
...
static PrintSegVInfo(void)
{
	...
	wsprintf(msg,
		 _T
		 ("Access violation at %08x. The memory could not be %s.\r\n"),
...
static void PrintStack(void)
{
	...
	__try {
		__asm {
			...
			mov eax, fs:[4]
		mov stacktop, eax} if (stacktop > stackptr + STACKTOPRINT)
