...
fixed_t angleturn[20] =
    { 0x32, 0x32, 0x53, 0x53, 0x64, 0x74, 0x85, 0x96, 0x96, 0xA6,
...
static CMD(SpawnThing)
{
	...
	x = player->mo->x + FixedMul(INT2F(64) + mobjinfo[id].radius,
				     dcos(player->mo->angle));
	y = player->mo->y + FixedMul(INT2F(64) + mobjinfo[id].radius,
				     dsin(player->mo->angle));
	z = player->mo->z;
	...
	if (pc->key[PCKEY_STRAFE]) {
		...
		if (forcefreelook != 2) {
			if ((int)v_mlook.value || forcefreelook)
				cmd->pitch -=
				    (int)v_mlookinvert.value ? pc->mousey *
				    0x8 : -(pc->mousey * 0x8);
		}
	}
...
dboolean G_Responder(event_t * ev)
{
	...
	if (gamestate == GS_LEVEL) {
		...
		if (demoplayback && gameaction == ga_nothing) {
			if (ev->type == ev_keydown || ev->type == ev_gamepad) {
				G_CheckDemoStatus();
	...
	for (i = 0; i < MAXPLAYERS; i++) {
		if (playeringame[i]) {
			if (players[i].cmd.buttons & BT_SPECIAL) {
				...
				if ((players[i].cmd.buttons & BT_SPECIALMASK) ==
				    BTS_PAUSE) {
					paused ^= 1;
					if (paused)
						S_PauseSound();
...
void G_PlayerReborn(int player)
{
	...
	dmemcpy(wpns, players[player].weaponowned,
		sizeof(dboolean) * NUMWEAPONS);
...
void G_DeathMatchSpawnPlayer(int playernum)
{
	...
	if (selections < 4)
		I_Error
		    ("G_DeathMatchSpawnPlayer: Only %i deathmatch spots, 4 required",
		     selections);
...
void G_DoReborn(int playernum)
{
	...
	else			// respawn at the start
	{
		...
		if (players[playernum].mo == NULL)
			I_Error("G_DoReborn: Player start #%i not found!",
				playernum + 1);
...
void G_RunGame(void)
{
	...
	while (gameaction != ga_title) {
		...
		if (next == ga_victory) {
			next =
			    D_MiniLoop(IN_Start, IN_Stop, IN_Drawer, IN_Ticker);
