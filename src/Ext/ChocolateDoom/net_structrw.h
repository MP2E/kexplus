...
extern dboolean NET_ReadSettings(net_packet_t * packet,
				 net_gamesettings_t * settings);

extern void NET_WriteQueryData(net_packet_t * packet,
			       net_querydata_t * querydata);
extern dboolean NET_ReadQueryData(net_packet_t * packet,
				  net_querydata_t * querydata);

extern void NET_WriteTiccmdDiff(net_packet_t * packet, net_ticdiff_t * diff,
				dboolean lowres_turn);
extern dboolean NET_ReadTiccmdDiff(net_packet_t * packet, net_ticdiff_t * diff,
				   dboolean lowres_turn);
extern void NET_TiccmdDiff(ticcmd_t * tic1, ticcmd_t * tic2,
			   net_ticdiff_t * diff);
extern void NET_TiccmdPatch(ticcmd_t * src, net_ticdiff_t * diff,
			    ticcmd_t * dest);

dboolean NET_ReadFullTiccmd(net_packet_t * packet, net_full_ticcmd_t * cmd,
			    dboolean lowres_turn);
