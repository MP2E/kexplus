...
static dboolean DL_ProcessAutomap(vtxlist_t * vl, int *drawcount)
{
	...
	if (am_ssect.value) {
		...
		color = D_RGBA((num * 0x3f) & 0xff,
			       (num * 0xf) & 0xff, (num * 0x7) & 0xff, 0xff);
		...
		if (nolights)
			...
			color = D_RGBA(light->active_r,
				       light->active_g, light->active_b, 0xff);
		}
	}
...
void AM_DrawLeafs(float scale)
{
	...
	for (i = 0; i < numsubsectors; i++) {
		...
		// must be mapped
		//
		if (segs[sub->firstline].linedef->flags & ML_MAPPED
		    || amCheating) {
			//
			// add to draw list if visible
			//
			if (!(sub->sector->flags & MS_HIDESSECTOR)
			    || am_fulldraw.value) {
				vtxlist_t *list;
...
void AM_DrawTriangle(mobj_t * mobj, float scale, dboolean solid, byte r, byte g,
		     byte b)
{
	vtx_t tri[3];
	fixed_t x;
...
void AM_DrawSprite(mobj_t * thing, float scale)
{
	...
	if (!thing->sprite)
		...
		if (sprframe->rotate)
			rot =
			    ((am_viewangle - thing->angle) + ANG90 +
			     (unsigned)(ANG45 / 2) * 9) >> 29;

		//
		// keys and artifacts are scaled when zooming out
		//
		if (thing->type >= MT_ITEM_BLUECARDKEY
		    && thing->type <= MT_ITEM_ARTIFACT3) {
		...
		height =
		    ((float)spriteheight[sprframe->lump[rot]] * scalefactor);
	...
	if ((thing->frame & FF_FULLBRIGHT) || nolights
	    || thing->flags & MF_SPECIAL || amModeCycle)
