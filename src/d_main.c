...
static void Credits_Drawer(void)
{
	...
	case 0:
		...
		Draw_GfxImage(40, 40, "IDCRED2",
			      D_RGBA(255, 255, 255, (byte) screenalphatext),
			      true);
		break;
...
void D_DoomLoop(void)
{
	...
	while (1) {
		exit =
		    D_MiniLoop(Title_Start, Title_Stop, Title_Drawer,
			       Title_Ticker);

		if (exit == ga_newgame || exit == ga_loadgame)
			G_RunGame();
		else {
			D_MiniLoop(Credits_Start, NULL, Credits_Drawer,
				   Credits_Ticker);
