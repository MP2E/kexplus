...
void P_RunMobjs(void)
{
	for (currentmobj = mobjhead.next; currentmobj != &mobjhead;
		...
		if (gameflags & GF_LOCKMONSTERS && !currentmobj->player
		    && currentmobj->flags & MF_COUNTKILL)
			continue;
...
static void P_UpdateFrameStates(void)
{
	...
	frame_angle =
	    (player->cameratarget->angle + quakeviewx) + viewangleoffset;
	frame_pitch = pitch;
	...
	for (mobj = mobjhead.next; mobj != &mobjhead; mobj = mobj->next) {
		if (!mobj) {
			CON_Warnf
			    ("P_UpdateFrameStates: Null mobj in linked list!\n");
			continue;
		}
		// Special case only
