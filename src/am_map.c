...
static void AM_GetBounds(void)
{
	...
	for (i = 0; i < 8; i += 2) {
		...
		x1 = bx *
		    finecosine[(ANG90 - automapangle) >> ANGLETOFINESHIFT];
	...
	if (am_flags & AF_PANMODE) {
		...
		if (am_flags & AF_PANGAMEPAD) {
			if (ev->data3 == XINPUT_GAMEPAD_LEFT_STICK) {
				...
				x = (float)ev->data1 *
				    i_rsticksensitivity.value / (1500.0f /
								 scale);
				y = (float)ev->data2 *
				    i_rsticksensitivity.value / (1500.0f /
								 scale);
		...
		if (ev->type == ev_keydown) {
			...
			switch (ev->data1) {
			case BUTTON_A:
				CMD_AutomapSetFlag(AF_PANGAMEPAD | PCKF_UP,
						   NULL);
				break;
			...
			case BUTTON_DPAD_DOWN:
				CMD_AutomapSetFlag(AF_PANBOTTOM | PCKF_UP,
						   NULL);
				break;
...
void AM_Ticker(void)
{
	...
	if (followplayer) {
		if (am_flags & AF_PANMODE) {
			int panscalex =
			    (int)(v_msensitivityx.value / (1500.0f / scale));
			int panscaley =
			    (int)(v_msensitivityy.value / (1500.0f / scale));

			automappanx +=
			    ((I_MouseAccel(mpanx) * panscalex) / 128) << 16;
			automappany +=
			    ((I_MouseAccel(mpany) * panscaley) / 128) << 16;
			mpanx = mpany = 0;
		} else {
			automapx = plr->mo->x;
			automapy = plr->mo->y;
...
void AM_DrawWalls(void)
{
	...
	for (i = 0; i < numlines; i++) {
		...
		if ((l->flags & ML_MAPPED) || am_fulldraw.value
		    || plr->powers[pw_allmap] || amCheating) {
			...
			//
				...
				if (am_showkeycolors.value) {
					if (l->special & MLU_RED) {
						color =
						    D_RGBA(0xFF, 0x00, 0x00,
							   0xFF);
					} else if (l->special & MLU_BLUE) {
						color =
						    D_RGBA(0x00, 0x00, 0xFF,
							   0xFF);
					} else if (l->special & MLU_YELLOW) {
						color =
						    D_RGBA(0xFF, 0xFF, 0x00,
							   0xFF);
					} else {
						//
...
void AM_drawPlayers(void)
{
	...
	if (!netgame) {
		...
		for (i = 0; i < MAXPLAYERS; i++) {
			...
			switch (i) {
			case 0:	// Green
				AM_DrawTriangle(p->mo, scale, amModeCycle, 0,
						flash, 0);
				break;
			case 1:	// Red
				AM_DrawTriangle(p->mo, scale, amModeCycle,
						flash, 0, 0);
				break;
			case 2:	// Aqua
				AM_DrawTriangle(p->mo, scale, amModeCycle, 0,
				...
				break;
			case 3:	// Blue
				AM_DrawTriangle(p->mo, scale, amModeCycle, 0, 0,
						flash);
				break;
			}
		}
	...
	for (i = 0; i < numsectors; i++) {
		...
		while (t) {
			...
			//
				...
				if (t->type >= MT_ITEM_BLUECARDKEY
				    && t->type <= MT_ITEM_ARTIFACT3) {
					byte r, g, b;
...
void AM_Drawer(void)
{
	...
	if (plr->artifacts) {
		...
		if (plr->artifacts & (1 << ART_TRIPLE)) {
			Draw_Sprite2D(SPR_ART3, 0, 0, x, 255, 1.0f, 0,
				      WHITEALPHA(0x80));
			x -= 40;
		}

		if (plr->artifacts & (1 << ART_DOUBLE)) {
			Draw_Sprite2D(SPR_ART2, 0, 0, x, 255, 1.0f, 0,
				      WHITEALPHA(0x80));
			x -= 40;
		}

		if (plr->artifacts & (1 << ART_FAST)) {
			Draw_Sprite2D(SPR_ART1, 0, 0, x, 255, 1.0f, 0,
				      WHITEALPHA(0x80));
			x -= 40;
		}
	}
...
void AM_RegisterCvars(void)
{
	...
	G_AddCommand("-automap_right", CMD_AutomapSetFlag,
		     AF_PANRIGHT | PCKF_UP);
	...
	G_AddCommand("-automap_down", CMD_AutomapSetFlag,
		     AF_PANBOTTOM | PCKF_UP);
	G_AddCommand("+automap_freepan", CMD_AutomapSetFlag, AF_PANMODE);
	G_AddCommand("-automap_freepan", CMD_AutomapSetFlag,
		     AF_PANMODE | PCKF_UP);
