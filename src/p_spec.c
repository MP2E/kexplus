...
static void P_InitAnimdef(void)
{
	...
	while (sc_parser.readtokens()) {
		...
		if (!dstricmp(sc_parser.token, "ANIMPIC")) {
			...
			sc_parser.find(false);
			dstrncpy(anim.name, sc_parser.token,
				 dstrlen(sc_parser.token));
			...
			animdefs = Z_Realloc(animdefs,
					     sizeof(animdef_t) * ++numanimdef,
					     PU_STATIC, 0);
			dmemcpy(&animdefs[numanimdef - 1], &anim,
				sizeof(animdef_t));
		} else
			sc_parser.error("P_InitAnimdef");
	}
...
void P_InitPicAnims(void)
{
	...
	animinfo =
	    (animinfo_t *) Z_Malloc(sizeof(animinfo_t) * numanimdef, PU_STATIC,
				    0);
	...
	for (i = 0; i < numanimdef; i++) {
		...
		animinfo[i].texnum =
		    W_GetNumForName(animdefs[i].name) - t_start;
		animinfo[i].frame = -1;
		...
		if (animdefs[i].palette) {
			...
			textureptr[lump] =
			    (dtexture *) Z_Realloc(textureptr[lump],
						   animdefs[i].frames *
						   sizeof(dtexture), PU_STATIC,
						   0);
		}
	}
}
...
void P_CyclePicAnims(void)
{
	...
	for (i = 0; i < numanimdef; i++) {
		...
		if (anim->palette)
			...
			texturetranslation[info->texnum] =
			    info->texnum + info->frame;
		...
		if (line1->tag == tag1) {
			...
			case modl_texture:
				sides[line1->sidenum[0]].bottomtexture =
				    sides[line2->sidenum[0]].bottomtexture;
				sides[line1->sidenum[0]].midtexture =
				    sides[line2->sidenum[0]].midtexture;
				sides[line1->sidenum[0]].toptexture =
				    sides[line2->sidenum[0]].toptexture;
				...
				if (line1->flags & ML_TWOSIDED
					...
					sides[line1->sidenum[1]].toptexture =
					    sides[line2->sidenum[1]].toptexture;
				}
...
int P_RandomLineTrigger(line_t * line, mobj_t * thing, int side)
{
	...
	linelist =
	    (line_t **) Z_Malloc(count * sizeof(line_t *), PU_LEVEL, NULL);
	randLine = linelist;
	...
	P_UseSpecialLine(thing, linelist[(P_Random(pr_randomtrigger) % count)],
			 side);
	Z_Free(linelist);
...
int P_SetAimCamera(player_t * player, line_t * line, dboolean aim)
{
	...
	for (mo = mobjhead.next; mo != &mobjhead; mo = mo->next) {
		...
		mo->angle =
		    R_PointToAngle2(mo->x, mo->y, player->mo->x, player->mo->y);
...
void T_MovingCamera(movecamera_t * camera)
{
	...
	//
	dist =
	    P_AproxDistance(camtarget->x - camera->x, camtarget->y - camera->y);
	if (dist >= (48 * FRACUNIT)) {
		camtarget->pitch = P_AdjustAngle(camtarget->pitch,
						 R_PointToPitch(camtarget->z,
								camera->z,
								dist),
						 CAMTRACEANGLE);
	}
...
int P_DoSpecialLine(mobj_t * thing, line_t * line, int side)
{
	...
	case 245:
		...
		ok = EV_DoCeiling(line, customCeilingToHeight,
				  CEILSPEED * 2048);
		break;
...
void P_SpawnSpecials(void)
{
	...
	for (i = 0; i < numspawnlist; i++) {
		if (spawnlist[i].type ==
		    mobjinfo[MT_ITEM_BLUESKULLKEY].doomednum)
			tryopentype[0] = it_blueskull;

		if (spawnlist[i].type ==
		    mobjinfo[MT_ITEM_YELLOWSKULLKEY].doomednum)
			tryopentype[1] = it_yellowskull;

		if (spawnlist[i].type ==
		    mobjinfo[MT_ITEM_REDSKULLKEY].doomednum)
			tryopentype[2] = it_redskull;
	}
	...
	for (i = 0; i < numlines; i++) {
		if (lines[i].flags & ML_SCROLLRIGHT
		    || lines[i].flags & ML_SCROLLLEFT
		    || lines[i].flags & ML_SCROLLUP
		    || lines[i].flags & ML_SCROLLDOWN) {
