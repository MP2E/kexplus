...
void WI_Start(void)
{
	...
	for (i = 0; i < MAXPLAYERS; i++) {
		...
		if (!totalkills)
			...
			killpercent[i] =
			    (players[i].killcount * 100) / totalkills;

		if (!totalitems)
			itempercent[i] = 100;
		else
			itempercent[i] =
			    (players[i].itemcount * 100) / totalitems;
...
int WI_Ticker(void)
{
	...
	if (wi_advance >= 4) {
		...
		if ((nextcluster && cluster != nextcluster
		     && nextcluster->enteronly) || (cluster
						    && cluster != nextcluster
						    && !cluster->enteronly))
			return ga_victory;
	...
	if (!netgame) {
		...
		for (i = 0; i < MAXPLAYERS; i++) {
			...
			if (wi_stage > 0 && killvalue[i] > -1)
				Draw_Number(180, y, killvalue[i], 1,
					    WIALPHARED);

			if (wi_stage > 1 && itemvalue[i] > -1)
				Draw_Number(300, y, itemvalue[i], 1,
					    WIALPHARED);

			if (wi_stage > 2 && secretvalue[i] > -1)
				Draw_Number(412, y, secretvalue[i], 1,
					    WIALPHARED);
