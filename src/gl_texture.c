...
static void InitWorldTextures(void)
{
	...
	textureptr =
	    (dtexture **) Z_Calloc(sizeof(dtexture *) * numtextures, PU_STATIC,
				   NULL);
	texturetranslation =
	    Z_Calloc(numtextures * sizeof(word), PU_STATIC, NULL);
	palettetranslation =
	    Z_Calloc(numtextures * sizeof(word), PU_STATIC, NULL);
	...
	for (i = 0; i < numtextures; i++) {
		...
		textureptr[i] =
		    (dtexture *) Z_Malloc(1 * sizeof(dtexture), PU_STATIC, 0);

		// get starting index for switch textures
		if (!dstrnicmp(lumpinfo[t_start + i].name, "SWX", 3)
		    && swx_start == -1)
			swx_start = i;
		...
		png =
		    I_PNGReadData(t_start + i, true, true, false, &w, &h, NULL,
				  0);
...
void GL_BindWorldTexture(int texnum, int *width, int *height)
{
	...
	if (textureptr[texnum][palettetranslation[texnum]]) {
		dglBindTexture(GL_TEXTURE_2D,
			       textureptr[texnum][palettetranslation[texnum]]);
	...
	dglBindTexture(GL_TEXTURE_2D,
		       textureptr[texnum][palettetranslation[texnum]]);
	dglTexImage2D(GL_TEXTURE_2D, 0, GL_RGBA8, w, h, 0, GL_RGBA,
...
static void SetTextureImage(byte * data, int bits, int *origwidth,
			    int *origheight, int format, int type)
{
	if (r_texnonpowresize.value > 0) {
		...
		if (r_texnonpowresize.value >= 2) {
			...
			gluScaleImage(type, *origwidth, *origheight,
				      GL_UNSIGNED_BYTE, data, wp, hp,
				      GL_UNSIGNED_BYTE, pad);
		} else {
			int y;

			for (y = 0; y < *origheight; y++) {
				dmemcpy(pad + y * wp * bits,
					((byte *) data) + y * *origwidth * bits,
					*origwidth * bits);
			}
		...
		dglTexImage2D(GL_TEXTURE_2D,
			      0,
			      format, wp, hp, 0, type, GL_UNSIGNED_BYTE, pad);
		...
		dglTexImage2D(GL_TEXTURE_2D,
			      ...
			      format,
			      *origwidth,
			      *origheight, 0, type, GL_UNSIGNED_BYTE, data);
	}
...
static void InitGfxTextures(void)
{
	...
	for (i = 0; i < numgfx; i++) {
		...
		png =
		    I_PNGReadData(g_start + i, true, true, false, &w, &h, NULL,
				  0);
...
static void InitSpriteTextures(void)
{
	...
	spriteoffset =
	    (float *)Z_Malloc(numsprtex * sizeof(float), PU_STATIC, 0);
	spritetopoffset =
	    (float *)Z_Malloc(numsprtex * sizeof(float), PU_STATIC, 0);
	spriteheight =
	    (word *) Z_Malloc(numsprtex * sizeof(word), PU_STATIC, 0);
	spriteptr =
	    (dtexture **) Z_Malloc(sizeof(dtexture *) * numsprtex, PU_STATIC,
				   0);
	...
	for (i = 0; i < numsprtex; i++) {
		...
		for (j = 0; j < NUMSPRITES; j++) {
			...
			if (!dstrncmp
			    (lumpinfo[s_start + i].name, sprnames[j], 4)) {
				char palname[9];

				// increase the count if a palette lump is found
				for (p = 1; p < 10; p++) {
					sprintf(palname, "PAL%s%i", sprnames[j],
						p);
	...
	for (i = 0; i < numsprtex; i++) {
		...
		spriteptr[i] =
		    (dtexture *) Z_Malloc(spritecount[i] * sizeof(dtexture),
					  PU_STATIC, 0);
		...
		png =
		    I_PNGReadData(s_start + i, true, true, false, &w, &h,
				  offset, 0);
...
void GL_BindSpriteTexture(int spritenum, int pal)
{
	...
	png =
	    I_PNGReadData(s_start + spritenum, false, true, true, &w, &h, NULL,
			  pal);
...
dtexture GL_ScreenToTexture(void)
{
	...
	dglTexImage2D(GL_TEXTURE_2D,
		      0,
		      GL_RGB8, width, height, 0, GL_RGB, GL_UNSIGNED_BYTE, 0);

	dglCopyTexSubImage2D(GL_TEXTURE_2D, 0, 0, 0, 0, 0, width, height);
...
void GL_BindDummyTexture(void)
{
	if (dummytexture == 0) {
		...
		dglTexImage2D(GL_TEXTURE_2D, 0, GL_RGB8, 4, 4, 0, GL_RGB,
...
void GL_BindEnvTexture(void)
{
	...
	if (envtexture == 0) {
		...
		dglTexImage2D(GL_TEXTURE_2D, 0, GL_RGBA8, 4, 4, 0, GL_RGBA,
			      GL_UNSIGNED_BYTE, (byte *) rgb);
		dglTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_WRAP_S, GL_REPEAT);
		dglTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_WRAP_T, GL_REPEAT);
...
void GL_UpdateEnvTexture(rcolor color)
{
	...
	dglTexSubImage2D(GL_TEXTURE_2D,
			 0,
			 0, 0, 4, 4, GL_RGBA, GL_UNSIGNED_BYTE, (byte *) rgb);
...
void GL_SetEnvColor(float *param)
{
	...
	if (f == NULL) {
		CON_Warnf
		    ("GL_SetEnvColor: passed in NULL for GL_TEXTURE_ENV_COLOR\n");
		return;
	}

	if (state->color[0] == f[0] &&
	    state->color[1] == f[1] &&
	    state->color[2] == f[2] && state->color[3] == f[3])
		return;
...
void GL_SetCombineSourceRGB(int source, int target)
{
	...
	dglTexEnvi(GL_TEXTURE_ENV, GL_SOURCE0_RGB + source,
		   state->source_rgb[source]);
}
...
void GL_SetCombineSourceAlpha(int source, int target)
{
	...
	dglTexEnvi(GL_TEXTURE_ENV, GL_SOURCE0_ALPHA + source,
		   state->source_alpha[source]);
}
...
void GL_SetCombineOperandRGB(int operand, int target)
{
	...
	dglTexEnvi(GL_TEXTURE_ENV, GL_OPERAND0_RGB + operand,
		   state->operand_rgb[operand]);
}
...
void GL_SetCombineOperandAlpha(int operand, int target)
{
	...
	dglTexEnvi(GL_TEXTURE_ENV, GL_OPERAND0_ALPHA + operand,
		   state->operand_alpha[operand]);
}
