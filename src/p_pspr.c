...
weaponinfo_t weaponinfo[NUMWEAPONS] = {
	{am_noammo, S_708, S_707, S_705, S_709, S_000}
	,			// chainsaw
	{am_noammo, S_714, S_713, S_712, S_715, S_000}
	,			// fist
	{am_clip, S_722, S_721, S_720, S_723, S_728}
	,			// pistol
	{am_shell, S_731, S_730, S_729, S_732, S_738}
	,			// shotgun
	{am_shell, S_741, S_740, S_739, S_742, S_752}
	,			// super shotgun
	{am_clip, S_755, S_754, S_753, S_756, S_759}
	,			// chaingun
	{am_misl, S_763, S_762, S_761, S_764, S_767}
	,			// rocket launcher
	{am_cell, S_773, S_772, S_771, S_775, S_000}
	,			// plasma gun
	{am_cell, S_783, S_782, S_781, S_784, S_788}
	,			// bfg
...
void A_Punch(player_t * player, pspdef_t * psp)
{
	...
	if (linetarget) {
		...
		player->mo->angle =
		    R_PointToAngle2(player->mo->x, player->mo->y, linetarget->x,
				    linetarget->y);
	}
}
...
static void P_LaserCrossBSP(int bspnum, laser_t * laser)
{
	...
	while (!(bspnum & NF_SUBSECTOR)) {
		...
		childlaser =
		    (laser_t *) Z_Calloc(sizeof(laser_t), PU_LEVSPEC, NULL);
...
void A_FireLaser(player_t * player, pspdef_t * psp)
{
	...
	for (i = 0; i < lasercount; i++) {
		...
		slope =
		    P_AimLineAttack(mobj, angleoffs, LASERAIMHEIGHT,
				    LASERRANGE);
		...
		laser[i]->x1 =
		    mobj->x + FixedMul(LASERDISTANCE, dcos(mobj->angle));
		laser[i]->y1 =
		    mobj->y + FixedMul(LASERDISTANCE, dsin(mobj->angle));
		...
		laserthinker[i] =
		    Z_Malloc(sizeof(*laserthinker[i]), PU_LEVSPEC, 0);
		P_AddThinker(&laserthinker[i]->thinker);

		laserthinker[i]->thinker.function.acp1 =
