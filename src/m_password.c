...
void M_EncodePassword(void)
{
	...
	M_EncodePassItem(&bit, player->ammo[am_shell],
	...
	*(short *)&encode[6] =
	    I_SwapBE16(~(decodebit[0] + decodebit[1] + decodebit[2]));
	*(short *)&encode[8] =
	    I_SwapBE16(~(decodebit[0] ^ decodebit[1] ^ decodebit[2]));
...
dboolean M_DecodePassword(dboolean checkOnly)
{
	...
	//
	player->ammo[am_clip] =
	    M_DecodePassItem(decode[2] >> 4, player->maxammo[am_clip]);
	player->ammo[am_shell] =
	    M_DecodePassItem(decode[2] & 0xf, player->maxammo[am_shell]);
	player->ammo[am_cell] =
	    M_DecodePassItem(decode[3] >> 4, player->maxammo[am_cell]);
	player->ammo[am_misl] =
	...
	//
	CON_CvarSetValue(p_features.name,
			 ((decode[0] >> 2) == 1) ? 1.0f : 0.0f);
