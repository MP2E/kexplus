		...
		if (!dfcmp(xr, delta)) {
			...
			if (!dfcmp(xg, delta)) {
				...
				sum =
				    ((((delta - xr) / (delta -
						       (min / 255.0f))) +
				      2.0f) - ((delta - (b / 255.0f)) / (delta -
									 (min /
									  255.0f))));
			}
		} else {
			sum =
			    (((delta - (b / 255.0f))) / (delta -
							 (min / 255.0f))) -
...
void R_SetLightFactor(float lightfactor)
{
	...
	for (l = 0; l < numlights; l++) {
		...
		if (l >= 256) {
			...
			R_LightGetRGB(h, s, v, (int *)&light->base_r,
				      (int *)&light->base_g,
				      (int *)&light->base_b);
		} else
			light->base_r = light->base_g = light->base_b =
			    MIN((int)((float)l * f), 255);
...
void R_RefreshBrightness(void)
{
	...
	factor =
	    (((infraredFactor >
	       brightness) ? infraredFactor : brightness) + 100.0f);
...
rcolor R_GetSectorLight(byte alpha, word ptr)
{
	return D_RGBA((byte) lights[ptr].active_r,
		      (byte) lights[ptr].active_g, (byte) lights[ptr].active_b,
		      alpha);
}
...
rcolor R_SplitLineColor(seg_t * line, byte side)
{
	...
	height =
	    (line->frontsector->ceilingheight -
	     line->frontsector->floorheight) / FRACUNIT;
	...
	if (side == 1)
		...
		sideheight2 =
		    (line->frontsector->ceilingheight -
		     line->backsector->ceilingheight) / FRACUNIT;
	} else if (side == 2)
		/*BOTTOM*/ {
		sideheight1 =
		...
		sideheight2 =
		    (line->frontsector->ceilingheight -
		     line->backsector->floorheight) / FRACUNIT;
		}
	...
	return D_RGBA((byte) MIN((r1 + r2), 0xff), (byte) MIN((g1 + g2), 0xff),
		      (byte) MIN((b1 + b2), 0xff), 0xff);
}
...
void R_SetSegLineColor(seg_t * line, vtx_t * v, byte side)
{
	...
	if (line->linedef->flags & ML_BLENDING) {
		if (line->backsector && side != 0) {
			if (!(line->linedef->flags & ML_BLENDFULLTOP)
			    && side == 1) {
				c[2] = R_SplitLineColor(line, 1);
				c[3] = c[2];
			} else {
				if (side == 1
				    && line->linedef->flags & ML_INVERSEBLEND)
					lwr = LIGHT_UPRWALL;

				c[2] = bspColor[lwr];
				c[3] = bspColor[lwr];
			}
			if (!(line->linedef->flags & ML_BLENDFULLBOTTOM)
			    && side == 2) {
