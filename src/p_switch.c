...
void P_ChangeSwitchTexture(line_t * line, int useAgain)
{
	...
	if (SPECIALMASK(line->special) == 52
	    || SPECIALMASK(line->special) == 124)
		sound = sfx_switch2;
	else
		sound = sfx_switch1;
	...
	if (SWITCHMASK(line->flags) == ML_SWITCHX04) {
		swx =
		    swx_start + (sides[line->sidenum[0]].bottomtexture -
				 swx_start) ^ 1;
		...
		swx =
		    swx_start + (sides[line->sidenum[0]].toptexture -
				 swx_start) ^ 1;
		...
		swx =
		    swx_start + (sides[line->sidenum[0]].midtexture -
				 swx_start) ^ 1;
