		...
		if (action) {
			...
			if (netgame) {
				if (cvar->nonclient) {
					...
					if (consoleplayer != 0) {
						CON_Warnf
						    ("Cannot change cvar that's locked by server\n");
						goto next;
					}
				}
			}
			...
			if (!al->param[0]) {
				char str[256];
				sprintf(str, "%s: %s (%s)", cvar->name,
					cvar->string, cvar->defvalue);
...
void G_ActionTicker(void)
{
	...
	for (slot = 0; slot < MAX_CURRENTACTIONS; slot++) {
		if (CurrentActions[slot])
			CurrentActions[slot] =
			    DoRunActions(CurrentActions[slot], true);
	}
...
dboolean G_ActionResponder(event_t * ev)
{
	...
	case ev_keydown:
		...
		// MP2E 12/11/2013: ev_mouse needs to process button actions
	case ev_mousedown:
	case ev_mouseup:
...
alist_t *ParseActions(char *actions)
{
	...
	while (true) {
		...
		al->next =
		    (alist_t *) Z_Malloc(sizeof(alist_t), PU_STATIC, NULL);
		al = al->next;
		al->buff = NULL;
	}
}
...
alist_t **G_FindKeyByName(char *key)
{
	...
	if (dstrncmp(key, "mouse", 5) == 0) {
		...
		if ((key[5] == '2') && key[6])
			return (FindActionControler
				(&key[6], Mouse2Actions, MOUSE_BUTTONS));
		return (FindActionControler
...
void G_DoOptimizeActionTree(void)
{
	...
	ActionBuffer =
	    (action_t **) Z_Malloc(count * sizeof(action_t *), PU_STATIC, NULL);
	NumActions = 0;
	DumpActions(Actions);
