...
static mobj_t *P_MissileAttack(mobj_t * actor, int direction)
{
	...
	mo = P_SpawnMissile(actor, actor->target, type, deltax, deltay, deltaz,
			    aim);
	...
	if (!actor->target || actor->target->health <= 0 ||
		...
		else		// special case for player bots
		{
			...
			for (mobj = mobjhead.next; mobj != &mobjhead;
				...
				dist =
				    P_AproxDistance(mobj->x - actor->x,
						    mobj->y - actor->y);
				if (!(dist < dist2))
					continue;
...
void A_RectMissile(mobj_t * actor)
{
	...
	mo = P_SpawnMobj(actor->x + x, actor->y + y, actor->z + 139 * FRACUNIT,
			 MT_PROJ_RECT);
	P_SetTarget(&mo->target, actor);
	P_SetTarget(&mo->tracer, actor->target);
	mo->threshold = 1;
...
void A_RectGroundFire(mobj_t * actor)
{
	...
	P_SetTarget(&mo->target, actor);
	an = actor->angle + R_PointToAngle2(actor->x, actor->y, mo->target->x,
					    mo->target->y);
...
void A_PainShootSkull(mobj_t * actor, angle_t angle)
{
	...
	for (newmobj = mobjhead.next; newmobj != &mobjhead;
	     newmobj = newmobj->next) {
	...
	if ((!P_TryMove(newmobj, newmobj->x, newmobj->y)) ||
	    (!P_PathTraverse
	     (actor->x, actor->y, newmobj->x, newmobj->y, PT_ADDLINES,
	      PIT_PainCheckLine))) {
		// kill it immediately
...
void A_BarrelExplode(mobj_t * actor)
{
	...
	exp =
	    P_SpawnMobj(actor->x + FRACUNIT, actor->y + FRACUNIT,
			actor->z + (actor->height << 1), MT_EXPLOSION1);
	A_Explode(actor);
