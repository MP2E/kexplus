...
{
	if (respawnspecials && ((mobj->flags & MF_SPECIAL)
...
void P_SpawnPlayerMissile(mobj_t * source, mobjtype_t type)
{
	...
	if (!linetarget) {
		...
		if (!linetarget) {
			an -= 2 << 26;
			slope =
			    P_AimLineAttack(source, an, missileheight,
					    ATTACKRANGE);
		}
	}
...
mobj_t *P_SpawnMissile(mobj_t * source, mobj_t * dest, mobjtype_t type,
		       fixed_t xoffs, fixed_t yoffs, fixed_t heightoffs,
		       dboolean aim)
{
	mobj_t *th;
	angle_t an;
