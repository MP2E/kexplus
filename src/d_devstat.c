...
void D_DeveloperDisplay(void)
{
	...
	Draw_Text(0, y, WHITE, 0.35f, false, "Zone PU_STATIC Usage: %6d kb",
		  Z_TagUsage(PU_STATIC) >> 10);
	y += 16;

	Draw_Text(0, y, WHITE, 0.35f, false, "Zone PU_LEVEL Usage: %7d kb",
		  Z_TagUsage(PU_LEVEL) >> 10);
	y += 16;

	Draw_Text(0, y, WHITE, 0.35f, false, "Zone PU_CACHE Usage: %7d kb",
		  Z_TagUsage(PU_CACHE) >> 10);
	y += 16;

	Draw_Text(0, y, WHITE, 0.35f, false, "Zone PU_LEVSPEC Usage: %5d kb",
		  Z_TagUsage(PU_LEVSPEC) >> 10);
	y += 16;

	Draw_Text(0, y, WHITE, 0.35f, false, "Zone PU_AUTO Usage: %8d kb",
		  Z_TagUsage(PU_AUTO) >> 10);
	y += 16;

	/*DRAW LIST INFORMATION */
	Draw_Text(0, y, WHITE, 0.35f, false, "Draw List WALL Usage: %6d kb",
		  DL_GetDrawListSize(DLT_WALL) >> 10);
	y += 16;

	Draw_Text(0, y, WHITE, 0.35f, false, "Draw List FLAT Usage: %6d kb",
		  DL_GetDrawListSize(DLT_FLAT) >> 10);
	y += 16;

	Draw_Text(0, y, WHITE, 0.35f, false, "Draw List SPRITE Usage: %4d kb",
		  DL_GetDrawListSize(DLT_SPRITE) >> 10);
	y += 16;

	Draw_Text(0, y, WHITE, 0.35f, false, "Draw List AMAP Usage: %6d kb",
		  DL_GetDrawListSize(DLT_AMAP) >> 10);
	y += 16;
	...
	if (gamestate == GS_LEVEL) {
		...
		Draw_Text(0, y, WHITE, 0.35f, false, "P_Mobj Total Things: %i",
			  p_nummobjthinkers);
		y += 16;
	}
	...
	Draw_Text(0, y, sevclr, 0.35f, false, "Texture Bind Calls: %i",
		  glBindCalls);
	y += 16;
	...
	if (gamestate == GS_LEVEL && !automapactive) {
		Draw_Text(0, y, WHITE, 0.35f, false,
			  "PlayerView Render Time: %ims", renderTic);
		y += 16;

		Draw_Text(0, y, WHITE, 0.35f, false, "Sprite Render Time: %ims",
			  spriteRenderTic);
		y += 16;
	}

	Draw_Text(0, y, WHITE, 0.35f, false, "Active Sounds: %i",
		  S_GetActiveSounds());
	y += 16;

	Draw_Text(0, y, WHITE, 0.35f, false, "Mouse Cursor: %i, %i", mouse_x,
		  mouse_y);
	y += 16;
