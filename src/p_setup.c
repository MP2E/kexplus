...
static void P_InitTextureHashTable(void)
{
	...
	for (i = 0; i < numtextures; i++) {
		texturehashlist[0][i] =
		    W_HashLumpName(lumpinfo[t + i].name) % 65536;
...
void P_LoadSectors(int lump)
{
	...
	for (i = 0; i < numsectors; i++, ss++, ms++) {
		...
		for (j = 0; j < numskydef; j++) {
			if (ss->ceilingpic ==
			    (W_GetNumForName(skydefs[j].flat) - t_start)) {
				skyflatnum = j;
				break;
			}
		}
...
void P_LoadMacros(int lump)
{
	...
	macros.def =
	    Z_Calloc(macros.macrocount * sizeof(macrodef_t), PU_LEVEL, NULL);
	...
	for (i = 0; i < macros.macrocount; i++) {
		...
		macros.def[i].data =
		    Z_Calloc(sizeof(macrodata_t) * count, PU_LEVEL, NULL);
		...
		if (ss->numleafs) {
			...
			for (j = 0; j < ss->numleafs; j++, lf++) {
				...
				if (vertex > numvertexes)
					I_Error
					    ("P_LoadLeafs: vertex out of range: %i - %i\n",
					     vertex, numvertexes);
				...
				if (seg == -1)
					...
					if (seg > numsegs) {
						if (!devparm)
							I_Error
							    ("P_LoadLeafs: seg out of range: %i - %i\n",
							     seg, numsegs);
						else
							CON_Warnf
							    ("P_LoadLeafs: seg out of range: %i - %i\n",
	...
	for (i = 0; i < numlines; i++, mld++, ld++) {
		...
		if (ld->special & MLU_MACRO) {
			if (MACROMASK(ld->special) >= macros.macrocount)
				I_Error
				    ("P_LoadLineDefs: linedef %i has unknown macro",
				     i);
		}
...
static void P_InitMapInfo(void)
{
	...
	while (sc_parser.readtokens()) {
		...
		if (!dstricmp(sc_parser.token, "MAP")) {
			...
			// read level name
			sc_parser.find(false);
			dstrncpy(mapdef.mapname, sc_parser.token,
			...
			while (sc_parser.readtokens()) {
				...
				if (!sc_parser.setdata(&mapdef, mapdatatable)) {
					...
					if (!dstricmp(sc_parser.token, "MUSIC")) {
						...
						ds_start =
						    W_GetNumForName("DS_START")
						    + 1;
						ds_end =
						    W_GetNumForName("DS_END") -
						    1;
						...
						if (lump > ds_end
						    && lump < ds_start) {
							CON_Warnf
							    ("P_InitMapInfo: Invalid music name: %s\n",
							     text);
							mapdef.music = -1;
						} else
							mapdef.music =
			...
			mapdefs = Z_Realloc(mapdefs,
					    sizeof(mapdef_t) * ++nummapdef,
					    PU_STATIC, 0);
			dmemcpy(&mapdefs[nummapdef - 1], &mapdef,
				sizeof(mapdef_t));
		}
		//
		// find cluster block
		//
			...
			clusterdefs = Z_Realloc(clusterdefs,
						sizeof(clusterdef_t) *
						++numclusterdef, PU_STATIC, 0);
			dmemcpy(&clusterdefs[numclusterdef - 1], &cluster,
				sizeof(clusterdef_t));
		} else
...
static void P_InitSkyDef(void)
{
	...
	while (sc_parser.readtokens()) {
		...
		if (!dstricmp(sc_parser.token, "SKY")) {
			...
			sc_parser.find(false);
			dstrncpy(sky.flat, sc_parser.token,
				 dstrlen(sc_parser.token));
				...
				if (!sc_parser.setdata(&sky, skydatatable)) {
					...
					if (!dstricmp(sc_parser.token, "CLOUD"))
						sky.flags |= SKF_CLOUD;
						 ...
						 (sc_parser.token, "THUNDER"))
						sky.flags |= SKF_THUNDER;
					else if (!dstricmp
						 (sc_parser.token, "FIRE"))
						sky.flags |= SKF_FIRE;
					else if (!dstricmp
						 (sc_parser.token,
						  "FADEINBACKGROUND"))
			...
			skydefs = Z_Realloc(skydefs,
					    sizeof(skydef_t) * ++numskydef,
					    PU_STATIC, 0);
			dmemcpy(&skydefs[numskydef - 1], &sky,
				sizeof(skydef_t));
		} else
			sc_parser.error("P_InitSkyDef");
	}
