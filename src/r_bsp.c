...
static void AddSegToDrawlist(drawlist_t * dl, seg_t * line, int texid,
			     int sidetype);
...
void R_AddClipLine(seg_t * line)
{
	...
	if (line->v1->validcount != validcount) {
		line->v1->clipspan =
		    R_PointToAngle2(line->v1->x, line->v1->y, viewx, viewy);
		line->v1->validcount = validcount;
	}

	if (line->v2->validcount != validcount) {
		line->v2->clipspan =
		    R_PointToAngle2(line->v2->x, line->v2->y, viewx, viewy);
		...
		if (line->backsector) {
			if ((line->frontsector->ceilingpic != skyflatnum &&
				...
				if ((line->backsector->floorheight ==
					...
					R_Clipper_SafeAddClipRange(angle2,
								   angle1);
			}
...
dboolean R_CheckBBox(fixed_t * bspcoord)
{
	...
	boxpos =
	    (viewx <= bspcoord[BOXLEFT] ? 0 : viewx <
	     bspcoord[BOXRIGHT] ? 1 : 2) + (viewy >=
					    bspcoord[BOXTOP] ? 0 : viewy >
					    bspcoord[BOXBOTTOM] ? 4 : 8);
	...
	angle1 =
	    R_PointToAngle2(bspcoord[check[0]], bspcoord[check[1]], viewx,
			    viewy) - viewangle;
	angle2 =
	    R_PointToAngle2(bspcoord[check[2]], bspcoord[check[3]], viewx,
			    viewy) - viewangle;
	...
	if (SWITCHMASK(line->linedef->flags) == ML_SWITCHX02) {
		...
		if (line->backsector) {
			if (line->backsector->floorheight >
			    line->frontsector->floorheight) {
				offset =
				    16 * FRACUNIT - (line->sidedef->rowoffset);
				top = line->backsector->floorheight - offset;
				bottom = top - (32 * FRACUNIT);
				...
				offset =
				    16 * FRACUNIT + (line->sidedef->rowoffset);
				bottom =
				    line->backsector->ceilingheight + offset;
				top = bottom + (32 * FRACUNIT);
			}
		} else
			return false;
...
d_inline static void GetSideTopBottom(sector_t * sector, rfloat * top,
				      rfloat * bottom)
{
...
static dboolean R_GenerateMiddleSegPlane(seg_t * line, vtx_t * v)
{
	...
	if (!
	    (line->linedef->flags & ML_SWITCHX02
	     && line->linedef->flags & ML_SWITCHX04)) {
		...
		if (linedef->flags & ML_DONTPEGMID && line->backsector) {
			v[0].tv = v[1].tv =
			    1 + rowoffs - ((top - btop) / height);
			v[2].tv = v[3].tv =
			    1 + rowoffs +
			    (((top + btop) - (bottom + bbottom)) / height) / 2;
		} else if (linedef->flags & ML_DONTPEGTOP && !line->backsector) {
			rowoffs =
			    ((F2D3D(sidedef->rowoffset) - bottom) -
			     (top - bottom)) / height;
...
static void R_AddLine(seg_t * line)
{
		...
		if (bottom < bbottom) {
			...
			if (line->sidedef[0].bottomtexture != 1) {
				if (R_FrustrumTestVertex(v, 4)) {
					AddSegToDrawlist(&drawlist[DLT_WALL],
							 line,
	...
	if (sidedef->midtexture != 1) {
		...
		if (!
		    (line->linedef->flags & ML_SWITCHX02
		     && line->linedef->flags & ML_SWITCHX04)) {
			AddSegToDrawlist(&drawlist[DLT_WALL], line,
					 sidedef->midtexture, 2);
			AddSwitchQuad(line);
		}
	}
}
...
void R_AllocSubsectorBuffer(void)
{
	...
	if (numverts <= 2)
		I_Error
		    ("R_AllocSubsectorBuffer: Subsector has incomplete vertices");
	...
	if (numverts > maxSubVerts) {
		...
		subsector_buffer =
		    (vtx_t *) Z_Malloc(numverts * sizeof(vtx_t), PU_STATIC,
				       NULL);
...
static void R_AddLeaf(subsector_t * sub)
{
	...
	if (sub->sector->floorpic != skyflatnum) {
		if (R_FrustrumTestVertex(subsector_buffer, sub->numleafs) &&
			...
			if (sub->sector->flags & MS_LIQUIDFLOOR) {
				AddLeafToDrawlist(dl, sub,
						  sub->sector->floorpic);
				dl->list[dl->index - 1].flags |= DLF_WATER1;

				AddLeafToDrawlist(dl, sub,
						  sub->sector->floorpic + 1);
	...
	if (sub->sector->ceilingpic != skyflatnum) {
		for (i = 0; i < sub->numleafs; i++) {
			...
			subsector_buffer[i].z =
			    F2D3D(sub->sector->ceilingheight);
