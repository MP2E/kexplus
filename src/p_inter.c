...
void P_TouchSpecialThing(mobj_t * special, mobj_t * toucher)
{
	...
	case SPR_MGUN:
		if (!P_GiveWeapon
		    (player, special, wp_chaingun, special->flags & MF_DROPPED))
			return;
	...
	case SPR_SHOT:
		if (!P_GiveWeapon
		    (player, special, wp_shotgun, special->flags & MF_DROPPED))
			return;
	...
	case SPR_SGN2:
		if (!P_GiveWeapon
		    (player, special, wp_supershotgun,
	...
	default:
		if (special->type != MT_FAKEITEM) {
			CON_Printf(YELLOW,
				   "P_SpecialThing: Unknown gettable thing: %s\n",
		  ...
		  int damage)
{
	...
	if (inflictor && !(target->flags & MF_NOCLIP) &&
		...
		ang =
		    R_PointToAngle2(inflictor->x, inflictor->y, target->x,
				    target->y);
	...
	if (target->health <= 0) {
		if (devparm && target->player
		    && target->player->cheats & CF_UNDYING) {
