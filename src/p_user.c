...
void P_SpectateMove(player_t * player, angle_t angle, angle_t pitch,
		    fixed_t move)
{
	fixed_t frac;
...
void P_UpdateFollowCamera(player_t * player)
{
	...
	camstatic->pitch =
	    R_PointToPitch(camstatic->z, height - 8 * FRACUNIT, dist);
}
...
void P_MovePlayer(player_t * player)
{
	...
	if (cmd->forwardmove) {
		if (player->cheats & CF_SPECTATOR)
			P_SpectateMove(player, player->mo->angle,
				       player->mo->pitch,
	...
	if (cmd->sidemove) {
		if (player->cheats & CF_SPECTATOR)
			P_SpectateMove(player, player->mo->angle - ANG90, 0,
				       cmd->sidemove * 2048);
		else if (player->onground)
...
void P_DeathThink(player_t * player)
{
	...
	if (player->attacker && player->attacker != player->mo) {
		...
		player->mo->angle =
		    P_AdjustAngle(player->mo->angle, angle, ANG5);
	...
	if (!((gametic - deathmocktics) < MAXMOCKTIME)) {
		player->message =
		    mockstrings[(P_Random(pr_playermock) % MAXMOCKTEXT)];
...
void P_PlayerThink(player_t * player)
{
	...
	if (player->mo->subsector->sector->flags &
	    (MS_SECRET |
	     MS_DAMAGEX5 | MS_DAMAGEX10 | MS_DAMAGEX20 | MS_SCROLLFLOOR)) {
	...
	if (cmd->buttons & BT_CHANGE) {
		...
		} else		// 20140211 villsa - new weapon cycle logic
		{
			dboolean direction;
			int weapon;
			...
			direction =
			    (cmd->buttons2 & BT2_NEXTWEAP) ? true : false;
			weapon = newweapon;
	...
	if (player->powers[pw_invisibility]) {
		if (player->powers[pw_invisibility] > 61
