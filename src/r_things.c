...
static void AddSpriteDrawlist(drawlist_t * dl, visspritelist_t * vis,
			      int texid);
...
void R_InitSprites(char **namelist)
{
	...
	spriteinfo =
	    Z_Malloc(numsprites * sizeof(*spriteinfo), PU_STATIC, NULL);
...
static void R_AddVisSprite(visspritelist_t * vissprite)
{
	...
	if (thing->sprite == SPR_SPOT && !(thing->flags & MF_RENDERLASER))
		return;
	...
	if (thing->flags & MF_RENDERLASER)
		...
		if (sprframe->rotate) {
			...
			ang =
			    R_PointToAngle(thing->x - viewx, thing->y - viewy);
			rot =
			    (ang - thing->angle +
...
static dboolean R_GenerateSpritePlane(visspritelist_t * vissprite,
				      vtx_t * vertex)
{
	float x;
	float y;
	...
	if (thing->flags & MF_NIGHTMARE)
		...
		dglSetVertexColor(vertex, D_RGBA(255, 255, 255, thing->alpha),
				  4);
	else {
		R_LightToVertex(vertex,
				thing->subsector->sector->colors[LIGHT_THING],
				4);
	}
...
static dboolean R_GenerateLaserPlane(visspritelist_t * vissprite,
				     vtx_t * vertex)
{
	float x;
	float y;
...
void R_SetupSprites(void)
{
	...
	for (vis = vissprite - 1; vis >= visspritelist; vis--) {
		...
		if (vis->spr->type >= MT_PROP_POLEBASELONG &&
			...
			vis->x =
			    F2D3D(vis->spr->x -
				  FixedMul(FLOATTOFIXED(1.5), dcos(ang)));
			vis->y =
			    F2D3D(vis->spr->y -
				  FixedMul(FLOATTOFIXED(1.5), dsin(ang)));
			vis->z =
			    F2D3D(R_Interpolate
				  (vis->spr->z, vis->spr->frame_z,
				   interpolate));
		} else		// normal vis sprite process
		{
			vis->x =
			    F2D3D(R_Interpolate
				  (vis->spr->x, vis->spr->frame_x,
				   interpolate));
			vis->y =
			    F2D3D(R_Interpolate
				  (vis->spr->y, vis->spr->frame_y,
				   interpolate));
			vis->z =
			    F2D3D(R_Interpolate
				  (vis->spr->z, vis->spr->frame_z,
...
void R_DrawPSprite(pspdef_t * psp, sector_t * sector, player_t * player)
{
	...
	x = (F2D3D
	     (R_Interpolate
	      (psp->sx, psp->frame_x, (int)i_interpolateframes.value))
	     - spriteoffset[spritenum]);

	y = (F2D3D
	     (R_Interpolate
	      (psp->sy, psp->frame_y, (int)i_interpolateframes.value))
...
void R_RenderPlayerSprites(player_t * player)
{
	...
	for (psp = player->psprites; psp < &player->psprites[NUMPSPRITES];
	     psp++) {
		if (psp->state)
			R_DrawPSprite(psp, player->mo->subsector->sector,
				      player);
	}
}
...
#define DRAWBBOXSIDE3(b1) \
	...
	for (i = 0; i < (vissprite - visspritelist); i++) {
		...
		if (thing->player
		    && thing->player->cameratarget == thing->player->mo)
			continue;
