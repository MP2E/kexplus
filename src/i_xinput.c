...
typedef dword(WINAPI * LPXINPUTSETRUMBLE) (dword userID,
					   xinputrumble_t * rumble);
...
void I_XInputPollEvent(void)
{
	...
	if (xgamepad.lMotorWindDown || xgamepad.rMotorWindDown) {
		...
		xgamepad.vibration.lMotorSpeed =
		    MAX(temp - xgamepad.lMotorWindDown, 0);

		temp = xgamepad.vibration.rMotorSpeed;
		xgamepad.vibration.rMotorSpeed =
		    MAX(temp - xgamepad.rMotorWindDown, 0);
	...
	if (ev->type == ev_gamepad) {
		...
		if (ev->data3 == XINPUT_GAMEPAD_LEFT_STICK) {
			...
			if (i_xinputscheme.value > 0) {
				...
				turnspeed =
				    i_rsticksensitivity.value /
				    i_rstickthreshold.value;

				if (ev->data1 != 0) {
					xgamepad.rxthreshold += turnspeed;

					if (xgamepad.rxthreshold >=
					    i_rsticksensitivity.value)
						xgamepad.rxthreshold =
						    i_rsticksensitivity.value;
				}
		...
		if (ev->data3 == XINPUT_GAMEPAD_RIGHT_STICK) {
			...
			turnspeed =
			    i_rsticksensitivity.value / i_rstickthreshold.value;
			...
			if (ev->data2 != 0) {
				...
				if (xgamepad.rythreshold >=
				    i_rsticksensitivity.value)
					xgamepad.rythreshold =
					    i_rsticksensitivity.value;
			}
...
void I_XInputInit(void)
{
	...
	if (hInst = LoadLibrary(XINPUT_DLL)) {
		...
		//
		I_XInputEnable =
		    (LPXINPUTENABLE) GetProcAddress(hInst, "XInputEnable");
		I_XInputGetState =
		    (LPXINPUTGETSTATE) GetProcAddress(hInst, "XInputGetState");
		I_XInputSetRumble =
		    (LPXINPUTSETRUMBLE) GetProcAddress(hInst, "XInputSetState");

		if (I_XInputEnable == NULL || I_XInputGetState == NULL
		    || I_XInputSetRumble == NULL)
			return;
